...
class condition extends \core_availability\condition {
    ...
    public function __construct($structure) {
        if (isset($structure->useragentes)) {
            $this->useragentes = $structure->useragentes;
        }
    }
    ...
    public function is_available($not, info $info, $grabthelot, $userid): bool {
        ...
        if (empty($this->useragentes)) {
            return !$not;
        }

        // Check if User-Agent matches the $this->useragentes regex
            $tmp = preg_quote($this->useragentes, '/');
        if (preg_match('/' . $this->useragentes . '/', $_SERVER['HTTP_USER_AGENT'])) {
            return !$not;
        }
    ...
    public function get_description($full, $not, info $info): string {
        ...
        if (has_capability('mod/assign:grade', $info->get_context())) {
            if ($this->is_available($not, $info, false, $USER->id)) {
                return get_string('require_condition_debug_ok', 'availability_useragent', $_SERVER['HTTP_USER_AGENT']);
            }

            return get_string('require_condition_debug_ko', 'availability_useragent', $_SERVER['HTTP_USER_AGENT']);
        }

        return get_string('require_condition', 'availability_useragent');
    }
    ...
    protected function get_debug_string(): string {
        return !empty($this->useragentes) ? 'useragentes ON' : 'useragentes OFF';
    }
     ...
     */
    public static function get_json(string $useragentes): \stdClass {
        return (object)[
                'type'        => 'useragent',
                'useragentes' => $useragentes,
        ];
    }
